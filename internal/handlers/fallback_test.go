...
func TestTryForwardWithConnection_Success(t *testing.T) {
	...
	err := h.tryForwardWithConnection(rec, "deepseek", "deepseek-chat", "conn-try", &ConnectionData{APIKey: "sk-try", BaseURL: srv.URL}, body, false, false, "/v1/chat/completions")
	if err != nil {
...
func TestTryForwardWithConnection_NoAPIKey(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	err := h.tryForwardWithConnection(rec, "deepseek", "deepseek-chat", "conn-x", &ConnectionData{}, []byte(`{}`), false, false, "/v1/chat/completions")
	if err == nil {
...
func TestHandleAccountFallback_RetryableLocksModel(t *testing.T) {
	...
	body := []byte(`{"model":"deepseek-chat","messages":[{"role":"user","content":"hi"}]}`)
	rec := httptest.NewRecorder()
	err := h.handleAccountFallback(rec, "deepseek", "deepseek-chat", "", body, false, false, "/v1/chat/completions")
	if err == nil {
...
func TestHandleAccountFallback_NoConnections(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	err := h.handleAccountFallback(rec, "nonexistent-provider", "model", "", body, false, false, "/v1/chat/completions")
	if err == nil {
