...
func ForwardOpencode(w http.ResponseWriter, req *Request) error {
	...
	if isOpencodeResponsesModel(cleanModel) {
		...
		// Gate free-tier memfingerprint nama tool quartet dalam huruf kecil;
		// varian Kapital dari Claude Code CLI di-rename di sini dan namanya
		// dipulihkan lagi pada respons (translator.ConcealFingerprintTools).
	...
	// Jalur Chat Completions opencode: penyamaran + pemulihan nama tool yang sama.
